...
This command allows you to destroy all instances mapped to the current application (based on EC2 Tags: ``app``, ``env``, ``role``, ``color``).
...
**************************
 ...
 The feature gives the possibility to choose the split strategy of the whole Auto Scaling instances and so create multiple instance groups. Each instance groups will be remove from their Load Balancer  before to
