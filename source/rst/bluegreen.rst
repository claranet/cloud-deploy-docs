...
7. When ready for `blue/green` deploy, trigger the `swapbluegreen` command to make the swap ! There will be two strategies available:
    ...
    * *isolated*: instances from the online pool are detached from the ELB, waiting for connection draining and then attach instances from the new pool to the ELB. /!\ A downtime is possible with this strategy, but it insures that the N version and N+1 version are never online together.
    * *bothversion*:  a contrario, this strategy will first attach instances form the new pool (N+1 version), wait for them to be `healthy` in the ELB, then detachold (N version) instances.
